...
- 답: repository, working directory, staging area
...
- 답: 1번은 새로운 커밋 E를 만들어 커밋들이 그대로 메인 브랜치로 병합, 2번은 하나의 커밋으로 메인 브랜치에 병합, 3번은 브랜치 커밋들의 베이스를 재설정해서 모두 새로운 커밋으로 변경한다.
...
- 답: 현재 작업 중인 위치는 develop, develop 브랜치의 가장 최근 커밋은 readme를 추가한 것이고, 그 커밋의 id가 a1s2d3f이다.
...
- 답: revert. reset은 커밋을 삭제하므로 커밋을 공유하는 다른 브랜치가 영향을 받을 수 있다. 따라서 커밋을 생성하여 되돌리는 revert가 더 안전하다.
